...
app.onSync((body) => {
    ...
    for (devicecounter = 0; devicecounter < deviceitems.length; devicecounter++) {
        // If device has 'TemperatureSetting'
        ...
        if (deviceitems[devicecounter].traits.includes('action.devices.traits.Modes')) {
            let modename = deviceitems[devicecounter].attributes.availableModes[0].name
            let modevalue = deviceitems[devicecounter].attributes.availableModes[0].settings[0].setting_name
            // noinspection JSIgnoredPromiseFromCall
